...
/**
 * LedgerSelectModal - Modal for selecting and managing ledgers
 */
export default function LedgerSelectModal({
    visible,
    onClose,
    ledgers,
    searchQuery,
                    ...
                    <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
                        <View style={styles.modalContainerFancy}>
                            ...
                            <ScrollView style={styles.listFancy} showsVerticalScrollIndicator={false}>
                                {ledgers.filter(l => l.name.toLowerCase().includes(searchQuery.toLowerCase())).map(l => (
                                    <TouchableOpacity
                                        key={l.id}
                                        style={[
                                            styles.listItemFancy,
                                            selectedId === l.id && !editMode && styles.listItemSelected
                                        ]}
                                        onPress={() => onSelect(l)}
                                        activeOpacity={0.7}
                                        disabled={editMode}
                                    >
                                        <View style={[styles.iconBoxFancy, selectedId === l.id && !editMode && { backgroundColor: theme.colors.primary }]}>
                                            <MaterialCommunityIcons name="book-variant" size={20} color={selectedId === l.id && !editMode ? "white" : theme.colors.primary} />
                                        </View>

                                        <View style={styles.listItemContent}>
                                            <Text style={[
                                                styles.listItemTitle,
                                                selectedId === l.id && !editMode && styles.listItemTitleSelected
                                            ]}>{l.name}</Text>
